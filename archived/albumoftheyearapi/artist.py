...
class ArtistMethods:
    ...
    def artist_albums(self, artist):
        ...
        albums = self.artist_page.find_all("div", class_="albumBlock")

        artist_albums = []
        for x in albums:
            album_name = x.find("div", class_="albumTitle").getText().strip()
    ...
    def artist_mixtapes(self, artist):
        url = self.artist_url + artist + "/?type=mixtape"
        if self.url != url:
            self.__set_artist_page(artist, url)

        mixtapes = self.artist_page.find_all("div", class_="albumBlock")

        artist_mixtapes = []
        for x in mixtapes:
            mixtape_name = x.find("div", class_="albumTitle").getText().strip()
    ...
    def artist_eps(self, artist):
        url = self.artist_url + artist + "/?type=ep"
        if self.url != url:
            self.__set_artist_page(artist, url)

        eps = self.artist_page.find_all("div", class_="albumBlock")

        artist_eps = []
        for x in eps:
            ep_name = x.find("div", class_="albumTitle").getText().strip()
    ...
    def artist_singles(self, artist):
        url = self.artist_url + artist + "/?type=single"
        if self.url != url:
            self.__set_artist_page(artist, url)

        singles = self.artist_page.find_all("div", class_="albumBlock")

        artist_singles = []
        for x in singles:
            single_name = x.find("div", class_="albumTitle").getText().strip()
