...
def setup(bot):
    @bot.command(name='mostlovedartists')
    async def mostlovedartists(ctx):
        async with ctx.message.channel.typing():
            await most_loved_artists(ctx)
            time.sleep(5)

    @bot.command(name='mla')
    async def mla(ctx):
        async with ctx.message.channel.typing():
            await most_loved_artists(ctx)
            time.sleep(5)

async def most_loved_artists(message):
    most_loved = get_most_loved_hated_artists('liked')
    leaderboard = []

    for idx, artist in enumerate(most_loved, start=1):
        artist_name = artist.get('artist_name')
        link = artist.get('link')
        liked_users_count = len(artist.get('liked_users', []))
    ...
    for i, page in enumerate(pages):
        ...
        embed = discord.Embed(title="Most Loved Artists", description=embed_description, color=embed_color)
