...
def setup(bot):
    @bot.command(name='ratingshaters', aliases=['rh'])
    async def ratingshaters(ctx):
        async with ctx.message.channel.typing():
            await get_ratings_haters(ctx)


async def get_ratings_haters(ctx):
    user_weighted_ratings, avg_rating = get_users_by_average_rating("worst")
    ...
    for i in range(0, total_users, 10):
        ...
        embed = discord.Embed(title=f"{ctx.guild.name} Haters Ranking", description=embed_description, color=discord.Color.red())
