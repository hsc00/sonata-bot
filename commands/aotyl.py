...
def setup(bot: commands.Bot):
    @bot.command(name='aotyl')
    async def aoty(ctx: commands.Context, year: Optional[str] = None, user_id: Optional[str] = None):
        async with ctx.typing():
            await get_worst_rated_releases(ctx, year, user_id)


async def get_worst_rated_releases(ctx: commands.Context, year: Optional[str], user_id: Optional[str] = None):
    user_id = ctx.author.id if user_id is None else format_user_id(user_id)
    user_id = int(user_id)
    
    username = await get_user_username(ctx, user_id)
    
    sorted_albums, target_year, avg_rating, error = await get_rym_aoty(ctx, user_id, year, "worst")  
    ...
    for i in range(0, len(sorted_albums), 10):
        ...
        embed = discord.Embed(
            title=f"{username.title()} Worst {target_year} Releases",
