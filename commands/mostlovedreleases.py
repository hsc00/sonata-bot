...
def setup(bot):
    @bot.command(name='mostlovedreleases')
...
async def most_loved_releases(message):
    most_loved = get_most_loved_hated_releases('liked')
    leaderboard = []
    ...
    for i, page in enumerate(pages):
        ...
        embed = discord.Embed(title="Most Loved Releases", description=embed_description, color=embed_color)
