...
import discord
from views import *
from API.ratings_cache import *
from API.rym_search import search_rym_release
from events import get_user_info

def setup(bot):
    @bot.command(name='randomrating')
    async def randomrating(ctx , user: str = None, extra: str = None):
        async with ctx.channel.typing():
            await get_random_rating_from_cache(ctx, user, extra)

    @bot.command(name='rdr')
    async def rdr(ctx , user: str = None, extra: str = None):
        async with ctx.channel.typing():
            await get_random_rating_from_cache(ctx, user, extra)

async def get_random_rating_from_cache(ctx, user, extra):
    if user == None:  
        rating = get_random_rating()
    else:
        if (user == 'glaze' or user == 'roast') and extra is None:
            action = user
            rating = get_rating_by_action(action)
        elif user and (extra == 'glaze' or extra == 'roast'):
            user = re.search(r'\d+', user).group()
            action = extra
            rating = get_rating_by_action(action, user)
        else:
            user = re.search(r'\d+', user).group()
            rating = get_rating_by_number_or_user_id(user)

        if 'error' in rating:
            await ctx.reply(rating['error'])
            return
    ...
    media_type = rating.get('media_type')
