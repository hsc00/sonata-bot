...
def setup(bot):
    @bot.command(name='mostratereleases', aliases=['mrr'])
    async def mostratedreleases(ctx):
        async with ctx.message.channel.typing():
            await most_rated_releases(ctx)
            time.sleep(5)

async def most_rated_releases(message):
    ...
    for idx, album in enumerate(most_rated, start=1):
        ...
        leaderboard.append(f"{idx}. [{artist_name} - {release_name}]({link}) (**{rating_count}** ratings)")
