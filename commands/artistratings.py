...
from utils.text_formatters import add_newline_limit
...
async def get_artist_ratings(ctx):
    ...
    search_result = search_rym_artist(release_query)
    ...
    for user, ratings in ratings_cache.items():
        if user == user_id:
            ...
            for rating in sorted_ratings:
                if (artist_name in rating.artist_name or artist_name in rating.artist_name_localized):
            ...
            if ratings_count > 10:
                ...
                for ratings_page in paginated_ratings:
                    ...
                    embeds.append(create_embed(ctx, paginated_ratings, embed_title, embed_description, user_ratings, average, link, embed_color, artist_img_url, ctx.author.name))
                user_ratings, ratings_count, average = [], 0, (0, 0)  # Reset user_ratings as a list

    if ratings_count:
        user_ratings = "".join(user_ratings)
        embeds.append(create_embed(ctx, None, embed_title, embed_description, user_ratings, average, link, embed_color, artist_img_url, ctx.author.name))
...
def create_embed(ctx, paginated_ratings, embed_title, embed_description, user_ratings, average, link, embed_color, artist_img_url, author_name):
    if average[1]:
        average = average[0] / average[1]
        embed_description += f"\n\n<@{ctx.author.id}> Average Rating: **{round(average, 2)}** ⭐\n\n{user_ratings}"
