...
from utils.embeds import make_lyrics_embed
...
class TracksCog(commands.Cog):
    ...
    async def lyrics(self, ctx: commands.Context):
        ...
        message = await ctx.send(f"⌛ Fetching lyrics for {artist_name} - {track_name}...")
        synced_lyrics = search(f"{artist_name} - {track_name}")

        if not synced_lyrics or synced_lyrics == "":
            await message.edit(content=f"❌ No lyrics found.")

        # Remove the timestamp from the lyrics
        lyrics_lines = [re.sub(r"^\[.+](.*)$", r"\1", line) for line in synced_lyrics.split('\n')]

        embed = make_lyrics_embed(lyrics_lines, track_name, artist_name)
